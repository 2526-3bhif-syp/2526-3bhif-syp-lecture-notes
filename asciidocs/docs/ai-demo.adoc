...
=== 5. `/opsx:archive`
Die abgeschlossenen Änderungen werden in die lebende Spezifikation übernommen. Der temporäre `changes/`-Ordner wird aufgeräumt und die Änderungen werden via Git committet:

[source,bash]
----
git add .
git commit -m "feat: implement vehicle crud endpoints with postgres persistence #1"
----

=== 6. Erstellung eines Continuation Prompts
Um bei einer neuen Session sofort wieder produktiv starten zu können, wird im Ordner `continuations/` ein Fortsetzungs-Prompt erstellt. Bei einem Neustart kann der Kontext über `/clear` gelöscht und der Continuation-Prompt eingelesen werden, was Token spart und Fehlinterpretationen verhindert.
