...
Work in pairs (Student A and student B in the sequel) and accomplish the following tasks
...
Play around with the GitHub UI to get used to it. Reflect the different directions when to compare the pulled changes.

==== Sync A Given Fork

1. A does some changes to `Readme.md`` commits and pushes
2. B reloads his fork to get the info that the original has changed
3. B syncs the changes

Again try the different scenarios GitHub offers (directly syncing, comparing) and get acquainted with the UI and the functionality.

==== Create a Pull Request from a Non-Synced Fork

1. A changes his repo and B reloads but **does not** sync!
2. B changes his repo (same file and line as A) and creates a pull request
...
Again: what happens? Prepare for a discussion next week about how to deal best with pull requests and especially which discipline to establish to avoid pain.